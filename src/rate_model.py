...
def sinr_pu(gp: float, gs: float, Pp: float, Psc: float, Psp: float, sigma2: float) -> float:
    ...
    Interpretation:
        The PU signal is interfered by both SU common and SU private streams.
    """
    denom = gs * (Psc + Psp) + sigma2
...
def sinr_su_private(gp: float, gs: float, Pp: float, Psc: float, Psp: float, sigma2: float) -> float:
    """
    SU private-stream SINR.

    Model:
        SINR_sp = gs * Psp / (gp * Pp + sigma2)

    Interpretation:
        The private stream is decoded after common-stream treatment,
        while PU interference remains.
    """
    denom = gp * Pp + sigma2
