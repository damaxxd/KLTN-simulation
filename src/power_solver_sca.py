...
from quality_model import summarize_quality, psnr_pu_from_rate, psnr_su_from_rate
...
def _surrogate_rates(
    ...
    # PU rate:
    #
    # SINR_p = gp Pp / (gs(Psc+Psp) + sigma2)
    #
    # R_p = c1B * log2(1 + SINR_p/c2)
    #     = c1B * [ log2( gp Pp + c2*(gs(Psc+Psp)+sigma2) )
    #               - log2( c2*(gs(Psc+Psp)+sigma2) ) ]
    # -----------------------------------------------------
    Ap = gp * Pp + c2_val * (gs * (Psc + Psp) + sigma2)
    Bp = c2_val * (gs * (Psc + Psp) + sigma2)

    Bp_k = c2_val * (gs * (Psc_k + Psp_k) + sigma2)
    ...
    # SU private rate:
    #
    # SINR_sp = gs Psp / (gp Pp + sigma2)
    #
    # R_sp = c1B * [ log2( gs Psp + c2*(gpPp + sigma2) )
    #                - log2( c2*(gpPp + sigma2) ) ]
    # -----------------------------------------------------
    Asp = gs * Psp + c2_val * (gp * Pp + sigma2)
    Bsp = c2_val * (gp * Pp + sigma2)

    Bsp_k = c2_val * (gp * Pp_k + sigma2)
    Rsp_lb = c1B * (_log2(Asp) - _lin_upper_log2(Bsp, Bsp_k))
...
def _objective_neg(
    ...
    SVC-aware QoE:
        QoE_p = PSNR_p
        QoE_s = PSNR_s + lambda_layer*(layers_sur/4) + lambda_rate*(Reff_sur/Rmax)
    ...
    # PSNR parts
    QoE_p = psnr_pu_from_rate(Rp_lb)
    PSNR_s_sur = psnr_su_from_rate(Reff_sur)

    # layer-aware reward
    layers_sur = _approx_layers_from_reff(Reff_sur)
    layer_bonus = lambda_layer * (layers_sur / 4.0)

    # optional rate-aware reward
    Rmax = float(svc_cum[-1])
    rate_bonus = lambda_rate * (Reff_sur / max(Rmax, 1e-9))

    QoE_s = PSNR_s_sur + layer_bonus + rate_bonus
...
def solve_power_sca(
    ...
    if phy["Rp"] < Rp_min or phy["Rsc"] < r_BL:
